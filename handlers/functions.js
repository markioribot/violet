    ...
    try {
        ...
        for (let i = 0; i < lyrics.length; i += 1000) {
            ...
            const embed = new Discord.MessageEmbed()
                ...
                .setFooter(`Música pedida por: ${song.user.tag}`, song.user.displayAvatarURL({dynamic: true}))
                .setColor(ee.color)
                .setDescription(current)
            embeds.push(embed);
        }
...
function onCoolDown(message, command) {
  ...
  function createBar(total, current, size = 25, line = "▬", slider = "🔷") {
    try {
      if (!total) throw "FALTA TEMPO MÁXIMO";
   ...
   */
   ...
   async function swap_pages(client, message, description, TITLE) {
     ...
     .setTitle(`${emoji.msg.ERROR} Nenhum conteúdo adicionado à função TROCAR PÁGINAS`)
     ...
     let swapmsg = await message.channel.send({   
         content: `***Clique no __Buttons__ para trocar as páginas***`,
         embeds: [embeds[0]], 
     ...
     collector.on('collect', async b => {
         if(b.user.id !== message.author.id)
           return b.reply(`<:declined:780403017160982538> **Apenas aquele que digitou ${prefix}ajuda pode reagir!**`, true)
           //page forward
           if(b.customId == "1") {
  ...
  function change_status(client) {
    try {
      client.user.setActivity(`${config.prefix}help ${config.prefix}Play | Ouça Suas Músicas do Spotify e YouTube`, {
        type: "LISTENING",
      });
    } catch (e) {
      console.log(String(e.stack).bgRed)
